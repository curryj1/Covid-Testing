...
export class AppointmentService {
  ...
  getAllAppointments(): Observable<any> {
    ...
    return this.http.get<AppointmentResponse>(this.AUTH_API + 'all', {
      responseType: 'json',
    });
  }

  getMyAppointments(person: Patient): Observable<any> {
    var data = this.http.get<AppointmentResponse>(
      this.AUTH_API + person.email,
      { responseType: 'json' }
    );
    console.log(data);
    return data;
  ...
  getEmployeeAppointment(person: Employee): Observable<any> {
    return this.http.get<AppointmentResponse>(
      this.AUTH_API + 'employee/' + person.email,
      { responseType: 'json' }
    );
  }
  ...
  getByCenter(center): Observable<any> {
    ...
    return this.http.get<AppointmentResponse>(
      this.AUTH_API + 'center/' + center.id,
      { responseType: 'json' }
    );
  }
  ...
  deleteAppointment(appointment): Observable<any> {
    return this.http.delete(
      this.AUTH_API + 'delete/' + appointment.id,
      httpOptions
    );
  }

  setResult(form): Observable<any> {
    return this.http.put(
      this.AUTH_API + 'update/' + form.id,
      {
        result: form.result,
      },
      httpOptions
